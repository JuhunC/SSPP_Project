...
var addChat = function(room, socket, message, callback){
	
}
var approveRoom = function(roomTitle, approval){
	var conditions = {
		title : roomTitle
	};
	...
	roomModel.findOneAndUpdate(conditions, update, function(e){
		if(e){
			throw(e);
		}
		else{
...
var getAllRooms = function(string,callback){
...
var getTitle = function(room, callback){
	find({'_id' : room.id}, function(err, result){
		if(err) throw err;
		else{
...
var getTitleById = function(roomId, callback){
	find({'_id' : roomId}, function(err, result){
		if(err) throw err;
		else{
			return callback(null, result[0].title);
		}
