...
var ioEvents = function (io) {
  ...
  io.of('/rooms').on('connection', function (socket) {
    ...
    socket.on('searchRoom', function(string){
      Room.getAllRooms(string, function(room){
  ...
  io.of('/chatroom').on('connection', function (socket) {
    ...
    socket.on('reload_all',function(roomId){
      console.log('Entered reload-all index.js');
      console.log(roomId);
      Room.findById(roomId, function (err, room) {
        console.log('reload_all part 2');
        console.log(room.title);
        if (err) throw err;
        if (!room) {
          ...
          Room.getUsers(room, socket, function (err, users, cuntUserInRoom) {
            console.log('reload_all part3');
            console.log(room);
            socket.broadcast.to(room.id).emit('reload');
            socket.emit('reload');
          });
        }
      });
      ...
      Room.findById(roomId, function (err, room) {
        ...
        if (!room) {
          ...
          Room.addUser(room, socket, function (err, newRoom) {
            ...
            Chat.getChats(newRoom, socket, function (err, msgs) {
              if (err) throw err;

              socket.emit('retrieveMessages', msgs);
            });

            Room.getTitle(newRoom, function(err, roomTitle){
              if (err) throw err;
              socket.emit('getRoomTitle', roomTitle);
            })
    ...
    socket.on('makeDeal', function(dealmsg){
      console.log(dealmsg.buyerId);
      var deal = {
        roomId : dealmsg.roomId,
        sellerId : dealmsg.sellerId,
        buyerId : dealmsg.buyerId,
        finishedDate: dealmsg.finishedDate
      };
      Deal.updateBuyer(
        deal,
        function(e){
    ...
    socket.on("getDeal", function(searchData, callback){
        Deal.getDeal(searchData, function(err, data){
          socket.emit("hereDeal", data);
        });
    });

    socket.on("dealSubmit", function(dealData, callback){
      Deal.create(dealData, function(err, newDeal){
        if(err) throw err;
        else {
  ...
  io.of('/deal').on('connection', function (socket) {

    socket.on("getTitle", function(roomId, callback){
      Room.getTitleById(roomId, function(err, title){
